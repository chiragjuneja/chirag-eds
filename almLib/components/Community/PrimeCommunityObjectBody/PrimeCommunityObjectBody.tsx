...
*/
import { getALMConfig, getAccessToken, getCsrfToken } from "../../../utils/global";
...
const PrimeCommunityObjectBody = (props: any) => {
  ...
  const getToken = () => {
    return csrfToken ? getCsrfToken() : getAccessToken();
  }

  const getTokenQp = () => {
    return (csrfToken ? 'csrf_token' : 'access_token');
  }
  ...
  const iframeSrc = `${
    ...
    object.id
  }&${getTokenQp() + `=${token}`}&player_type=inline`;
